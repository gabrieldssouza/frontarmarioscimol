...
const checkStatus = async (id, arma) => {
  ...
  try {
    ...
    const response = await axios.get(`https://apiarmarios.vercel.app/alocacao/id/${id}`);
...
useEffect(() => {
  ...
  async function editStatus(data, arma) {
    ...
    try {
      const response = await axios.put('https://apiarmarios.vercel.app/editararmario/:id', newdata);
      console.log('Bem editado com sucesso:', response.data);
    } catch (error) {
      console.error('Erro ao editar bem:', error);
    }
  ...
  const handleDesalocar = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://apiarmarios.vercel.app/alocacao/${id}`);
